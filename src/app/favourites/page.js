...
function Favourites() {
  ...
  useEffect(() => {
    ...
    const favoriteModels = photos.filter(model => savedFavorites.includes(model.name));
    ...
    <div className='min-h-screen flex flex-col'>
      ...
      <div className='hidden lg:flex lg:w-full '>
          <div className='lg:flex lg:w-2/3 2xl:w-2/3 mx-auto justify-between lg:pt-10 lg:pb-8 xl:pt-16 xl:pb-12 2xl:py-12'>
            ...
            <div className={`hidden lg:flex lg:text-5xl xl:text-7xl w-1/3 justify-center items-center text-zinc-800 cursor-default ${playfairDisplay.className}`}>FAVOURITES</div>
            <div className=' text-zinc-400 flex w-1/3 justify-end items-center '>
              ...
              <Link href={"/favourites"}>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="size-7 xl:size-8 mt-1 cursor-pointer text-zinc-800">
      ...
      <div className='lg:hidden'>
          <div className='border-b-2 border-zinc-200 w-full flex text-zinc-600' style={{height:60}}>
              <div className='flex justify-between items-center pl-5 pr-2' style={{width:size.width-60}}>
              ...
              <div className='border-l-2 border-zinc-200 flex justify-center items-center' style={{width:60}}>
                  {/* MENU */}
                  <RightSideMenu exactStyling="text-gray-600 w-[24px] h-[24px] mt-1"/>
              </div>
          </div>
          <div className={`flex justify-center text-4xl mt-3 mb-4 md:text-4xl md:mt-4 md:mb-5 text-zinc-800 cursor-default ${playfairDisplay.className}`}>FAVOURITES</div>
      </div>

      {/* Favorites Display */}
      <div className='flex-grow flex flex-col'>
        {favorites.length === 0 ? (
          <div className={`text-center text-zinc-300 text-4xl lg:text-7xl 2xl:text-9xl m-auto ${playfairDisplay.className}`}>No favorites added yet.</div>
        ) : (
          <div className='mt-2 lg:mt-0 2xl:w-5/6 grid mx-auto space-x-2 xl:mb-4'>
            <div className='grid md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-1 mx-2'>
              {favorites.map((model, n) => (
                <a className='z-10' href={`/women/${model.name}`} key={n}>
                  <Image
